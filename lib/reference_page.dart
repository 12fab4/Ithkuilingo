    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        MapEntry<String, String> element =
            widget.tutorials.entries.elementAt(index); // we get the title:content pair for this button
        // and create a button
        return TextButton(
          child: Align(
            ...
            child: Hero(
              tag: element.key, // with the title as a key (that means we assume all titles are unique)
              // this wrapper ensures all styles are applied for the animation
              child: Material(
                color: Colors.transparent,
                child: Text(element.key), // the actual visible Widget is a Label with the title
              ),
            ),
          ),
...
LinkedHashMap<String, String> readDir(Directory dir) {
  LinkedHashMap<String, String> map = LinkedHashMap(); // initializes an empty hashmap !with insertion order!
  var files = dir.listSync(recursive: false); // returns a list of all files in a folder (synchronously -> waits)
  files.sort((a, b) => a.path.compareTo(b.path)); // sorts the files by name -> 00.md listed before 01.md usw...
  for (var file in files) {
    // for every file
    if (file is File) {
      // ensures no directories are accidentally added
      String content = file.readAsStringSync(); // read the file content synchronously again
      String title = content.split("\n").first; // retrieve the first line as a title
      map[title] = content.replaceFirst(title, ""); // save the file in the hashmap as title: rest_of_content
    }
  }
  return map;
}
